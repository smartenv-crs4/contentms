...
function involve(req, res, type) {
  ...
  if(!uid) {res.boom.badRequest('Missing user id');}
    ...
    involvments.add(req.params.pid, uid, type)
    .then((r) => {res.json(r)})
    .catch((e) => {
      if(e.status === 409)
...
function involved(req, res, type) {
  ...
  if(!uid) {res.boom.badRequest('Missing user id');}
    ...
    involvments.exists(req.params.pid, uid, type)
    .then(isInvolved => {
      let r = {};
      r[type] = isInvolved;
      res.json(r)
...
function uninvolve(req, res, type) {
  ...
  if(!uid) {res.boom.badRequest('Missing user id');}
    ...
    involvments.delete(req.params.pid, uid, type)
    .then((r) => {res.json(r)})
    .catch((e) => {
      console.log(e)
      switch(e.status) {
...
function count(req, res, type) {
  ...
  involvments.countByType(pid, type)
  ...
  rate : (req, res, next) => {
    ...
    if(!uid) {res.boom.badRequest('Missing user id');}
      ...
      if(rate == undefined || !validator.isInt(rate, { min: 1, max: 5 })) {
        ...
        involvments.rate(req.params.pid, uid, rate)
        .then((r) => {res.json(r)})
        .catch((e) => {
          console.log(e);
          res.boom.badImplementation(e.error);
